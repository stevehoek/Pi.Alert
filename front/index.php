  ...
  $('#tableDevices').DataTable({
    ...
    'columnDefs'   : [
      ...
      {targets: [8],
        'createdCell': function (td, cellData, rowData, row, col) {
          if (cellData == 1){
            $(td).html ('<i data-toggle="tooltip" data-placement="right" title="Random MAC" style="font-size: 16px;" class="text-blue glyphicon glyphicon-random"></i>');
          } else {
            $(td).html ('');
          }
      } },
...
function getDevicesList (status) {
  ...
  switch (deviceStatus) {
    ...
    case 'new':        tableTitle = 'New Devices';         color = 'orange';  break;
